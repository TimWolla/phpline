...
class KeyMap {
	...
	private static function bindArrowKeys(KeyMap $map) {
		...
		self::_bind( $map, "\033[OA", Operation::PREVIOUS_HISTORY );
		self::_bind( $map, "\033[OB", Operation::NEXT_HISTORY );
		self::_bind( $map, "\033[OC", Operation::FORWARD_CHAR );
		self::_bind( $map, "\033[OD", Operation::BACKWARD_CHAR );
		self::_bind( $map, "\033[OH", Operation::BEGINNING_OF_LINE );
		self::_bind( $map, "\033[OF", Operation::END_OF_LINE );
